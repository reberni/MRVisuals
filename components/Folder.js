...
import deleteDataWithId from "./functions/deleteDataWithId";
export default function Folder({ name, onClick, id }) {
    ...
    return (
        ...
        <div className="flex items-center border border-gray-200 rounded-lg mb-2 p-2 shadow-sm cursor-pointer hover:shadow-md transition-all justify-between" onClick={() => onClick('path', name)}>
            ...
            <GrFormClose className="text-gray-100 hover:text-red-400 text-lg" onClick={() => deleteDataWithId(id)} />
        </div>
    )
